...
filtered_gibushon_civil_diff = gibushon_civil %>%
  ...
  mutate(tkufatit_14_zscore = ifelse(date.tkufatit_14_diff>=119 & date.tkufatit_14_diff<=937,tkufatit_14_zscore,NA),#169,943
         tkufatit_15_zscore = ifelse(date.tkufatit_15_diff>=77 & date.tkufatit_15_diff<=1557,tkufatit_15_zscore,NA),#380,1367
...
#         final.score.2018_zscore = ifelse(date.final.score.2018_diff>=124 & date.final.score.2018_diff<=1580,final.score.2018_zscore,NA),#1501,2437
         cf_2018 = ifelse(date.cf_2018_diff>=309 & date.cf_2018_diff<=1737,cf_2018,NA),#1128,2308
#         row_score_2019_zscore = ifelse(date.row_score_2019_diff>=256 & date.row_score_2019_diff<=1773,row_score_2019_zscore,NA),#1187,2041
         am_2015 = ifelse(date.am_2015_diff>=29 & date.am_2015_diff<=1403,am_2015,NA),#331,1477
         am_2018 = ifelse(date.am_2018_diff>=26 & date.am_2018_diff<=2284,am_2018,NA))#243,2401
...
write.xlsx (gibushon_final_filtered_corr_output,file = "C:/Users/USER/Documents/MAMDA/gibushon/gibushon_final_filtered_p_p_corr_output.xlsx")


# Correlations criteria-criteria

gibushon_final_filtered_relevant_predictors_columns_for_correlations <- gibushon_final_filtered[c(1060:1066,799:801,806,852)]
gibushon_final_filtered_relevant_predictors_columns_names_for_correlations <- c(colnames(gibushon_final_filtered[c(1060:1066,799:801,806,852)]))
gibushon_final_filtered_relevant_criteria_columns_for_correlations <- gibushon_final_filtered[c(1060:1066,799:801,1072,1030,852)]
gibushon_final_filtered_relevant_criteria_columns_names_for_correlations <- c(colnames(gibushon_final_filtered[c(1060:1066,799:801,1072,1030,852)]))
gibushon_final_filtered_corr_output<-data.frame()[16,]
...
write.xlsx(gibushon_final_filtered_corr_output,file = "C:/Users/USER/Documents/MAMDA/gibushon/gibushon_final_filtered_c_c_corr_output.xlsx")
...
library(ppcor)
gibushon_final_filtered_relevant_predictors_columns_for_spcorrelations <- gibushon_final_filtered[c(810,853:861,864:869,1031:1034)]
gibushon_final_filtered_relevant_predictors_columns_names_for_spcorrelations <- c(colnames(gibushon_final_filtered[c(810,853:861,864:869,1031:1034)]))
gibushon_final_filtered_relevant_criteria_columns_for_spcorrelations <- gibushon_final_filtered[c(1060:1066,799:801,1072,1030,852)]
gibushon_final_filtered_relevant_criteria_columns_names_for_spcorrelations <- c(colnames(gibushon_final_filtered[c(1060:1066,799:801,1072,1030,852)]))
...
write.xlsx(gibushon_final_filtered_spcorr_output,file = "C:/Users/USER/Documents/MAMDA/gibushon/gibushon_final_filtered_spcorr_output_p-c.xlsx")

# Correlations predictors-criteria on gibushon_final_filtered by various filters
...
counter = gibushon_final_filtered_reg %>%
  rowwise() %>%
  mutate(tkufatit_nna = sum(!is.na(c(final.score.2015_zscore,final.score.2017_zscore,tkufatit_14_zscore,final.score.2018_zscore,row_score_2019_zscore))),
         am_nna = sum(!is.na(c(am_2015,am_2018,am_2018_special))))
...
f <- 1
l <- 1
for (i in gibushon_final_filtered_reg_restriction_predictors) {
...
gibushon_final_filtered_reg_religion_non_Jewish = gibushon_final_filtered_reg %>%
  filter(religion==2)
cor.test(as.numeric(gibushon_final_filtered_reg_religion_non_Jewish$FinalGradeg_zscore),as.numeric(gibushon_final_filtered_reg_religion_non_Jewish$tkufatitam),use="pairwise.complete.obs")
cor.test(as.numeric(gibushon_final_filtered_reg_religion_non_Jewish$FinalGradeg_zscore),as.numeric(gibushon_final_filtered_reg_religion_non_Jewish$course_score_zscore),use="pairwise.complete.obs")

# religion - range restriction - Jewish
...
gibushon_civil_filtered_reg_religion_Jewish = gibushon_civil_filtered_reg %>%
  filter(religion==1)
gibushon_civil_filtered_reg_religion_Jewish_restriction_predictors = gibushon_civil_filtered_reg_religion_Jewish %>%
  select(FinalGradeg_zscore)
gibushon_civil_filtered_reg_religion_Jewish_restriction_predictors <- as.data.frame(gibushon_civil_filtered_reg_religion_Jewish_restriction_predictors)
# gibushon_final_filtered_reg_restriction_criteria = gibushon_final%>%
gibushon_final_filtered_reg_religion_Jewish_restriction_criteria = gibushon_final_filtered_reg_religion_Jewish%>%
  select(tkufatit,am,tkufatitam,course_score_zscore)

counter = gibushon_final_filtered_reg_religion_Jewish %>%
  rowwise() %>%
  mutate(tkufatit_nna = sum(!is.na(c(final.score.2015_zscore,final.score.2017_zscore,tkufatit_14_zscore,final.score.2018_zscore,row_score_2019_zscore))),
         am_nna = sum(!is.na(c(am_2015,am_2018,am_2018_special))))
...
for (i in gibushon_final_filtered_reg_religion_Jewish_restriction_predictors) {
  for (j in gibushon_final_filtered_reg_religion_Jewish_restriction_criteria) {
    corr_temp<-c()
    corr_try <- try(cor.test(as.numeric(i),as.numeric((j),use="pairwise.complete.obs"), silent=T))
    corr_temp$"predictor" <-ifelse(class(corr_try)=="try-error", NA, corr_try$estimate)
    corr_temp$p.value <-ifelse(class(corr_try)=="try-error", NA, corr_try$p.value)
    ...
    library (descr)
    Sxn <- round(describe (gibushon_civil_filtered_reg_religion_Jewish_restriction_predictors[f]),2)
    Sxn <- Sxn$sd
    Sxn
    ...
    ryy <- ifelse(names(gibushon_final_filtered_reg_religion_Jewish_restriction_criteria[k])=="tkufatit",0.623,
                  ifelse(names(gibushon_final_filtered_reg_religion_Jewish_restriction_criteria[k])=="am",0.477,0.710))
    
    # n=the avarage number of times the criterion was measured in this study
    # only for am and tkufatit.
    
    n <- ifelse(names(gibushon_final_filtered_reg_religion_Jewish_restriction_criteria[k])=="tkufatit",n_tkufatit,
                ifelse(names(gibushon_final_filtered_reg_religion_Jewish_restriction_criteria[k])=="am",n_am,NA))
    
    ryyb <- (ryy*n)/(1+(n-1)*ryy)
    
    rn2 <- ifelse(names(gibushon_final_filtered_reg_religion_Jewish_restriction_criteria[k])=="tkufatit" |
                    names(gibushon_final_filtered_reg_religion_Jewish_restriction_criteria[k])=="am",round(rn/sqrt(1*ryyb),2),
                  round(rn/sqrt(1*ryy),2))
    
    library(data.table)
    range_restriction_table = data.table(c(round(r0,2),round(rn,2),round(rn2,2)))
    range_restriction_table <- as.data.frame(range_restriction_table)
    print(names(gibushon_final_filtered_reg_religion_Jewish_restriction_predictors[l]))
    colnames(range_restriction_table)<-names(gibushon_final_filtered_reg_religion_Jewish_restriction_criteria[k])
    rownames(range_restriction_table) <- c("r0","rn","rn2")
    print(range_restriction_table)
    k <- k+1
  }
...
# religion - range restriction - non Jewish
...
# gibushon_final_filtered_reg_restriction_criteria = gibushon_final%>%
gibushon_final_filtered_reg_religion_non_Jewish_restriction_criteria = gibushon_final_filtered_reg_religion_non_Jewish%>%
  select(tkufatit,am,tkufatitam,course_score_zscore)

counter = gibushon_final_filtered_reg_religion_non_Jewish %>%
  rowwise() %>%
  mutate(tkufatit_nna = sum(!is.na(c(final.score.2015_zscore,final.score.2017_zscore,tkufatit_14_zscore,final.score.2018_zscore,row_score_2019_zscore))),
         am_nna = sum(!is.na(c(am_2015,am_2018,am_2018_special))))
...
f <- 1
l <- 1
for (i in gibushon_final_filtered_reg_religion_non_Jewish_restriction_predictors) {
  for (j in gibushon_final_filtered_reg_religion_non_Jewish_restriction_criteria) {
    corr_temp<-c()
    corr_try <- try(cor.test(as.numeric(i),as.numeric((j),use="pairwise.complete.obs"), silent=T))
    corr_temp$"predictor" <-ifelse(class(corr_try)=="try-error", NA, corr_try$estimate)
    corr_temp$p.value <-ifelse(class(corr_try)=="try-error", NA, corr_try$p.value)
    ...
    library (descr)
    Sxn <- round(describe (gibushon_civil_filtered_reg_religion_non_Jewish_restriction_predictors[f]),2)
    Sxn <- Sxn$sd
    Sxn
    ...
    ryy <- ifelse(names(gibushon_final_filtered_reg_religion_non_Jewish_restriction_criteria[k])=="tkufatit",0.623,
                  ifelse(names(gibushon_final_filtered_reg_religion_non_Jewish_restriction_criteria[k])=="am",0.477,0.710))
    
    # n=the avarage number of times the criterion was measured in this study
    # only for am and tkufatit.
    
    n <- ifelse(names(gibushon_final_filtered_reg_religion_non_Jewish_restriction_criteria[k])=="tkufatit",n_tkufatit,
                ifelse(names(gibushon_final_filtered_reg_religion_non_Jewish_restriction_criteria[k])=="am",n_am,NA))
    
    ryyb <- (ryy*n)/(1+(n-1)*ryy)
    
    rn2 <- ifelse(names(gibushon_final_filtered_reg_religion_non_Jewish_restriction_criteria[k])=="tkufatit" |
                    names(gibushon_final_filtered_reg_religion_non_Jewish_restriction_criteria[k])=="am",round(rn/sqrt(1*ryyb),2),
                  round(rn/sqrt(1*ryy),2))
    
    library(data.table)
    range_restriction_table = data.table(c(round(r0,2),round(rn,2),round(rn2,2)))
    range_restriction_table <- as.data.frame(range_restriction_table)
    print(names(gibushon_final_filtered_reg_religion_non_Jewish_restriction_predictors[l]))
    colnames(range_restriction_table)<-names(gibushon_final_filtered_reg_religion_non_Jewish_restriction_criteria[k])
    rownames(range_restriction_table) <- c("r0","rn","rn2")
    print(range_restriction_table)
    k <- k+1
  }
...
round(describe(as.numeric(unlist(gibushon_final_filtered$amcourses))),2)
round(freq(ordered(as.numeric(unlist(gibushon_civil_filtered$FinalGradeg))), plot = F,main=colnames(gibushon_civil_filtered$FinalGradeg),font=2),2)
